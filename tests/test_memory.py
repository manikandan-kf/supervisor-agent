...
Everything in this store is read back into a later turn's routing prompt, which
makes it an instruction channel as well as a data one. Two bounds hold it: an
allowlist on what may be written at all (memory poisoning, solution §04), and a
retention ceiling on how long a written value keeps driving decisions
(guardrail layer 3 — memory expiry). These pin both.
"""
