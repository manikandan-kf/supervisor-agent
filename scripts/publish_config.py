...
record, `src/supervisor/config_store.py` reads it at runtime, and there is
...
can actually use rather than a plain UC Delta one.
    ...
    python scripts/publish_config.py --list

    # would this publish change anything?  (default: nothing is written)
    python scripts/publish_config.py --diff

    # publish the bundled YAML as new versions and make them active
    python scripts/publish_config.py --apply
    python scripts/publish_config.py --apply --only guardrails -m "tighten injection rule"

    # roll one document back to an earlier version
    python scripts/publish_config.py --pin guardrails=3
...
def _repo_root() -> Path:
    ...
    upstream failure. Same helper as `register_prompts.py` and
    `deploy/log_and_deploy.py`; keep them identical.
    """
    try:
        here = Path(__file__)
    except NameError:
...
from supervisor.config_store import (  # noqa: E402
    CONFIG_NAMES,
    ConfigError,
    ConfigStore,
    checksum_of,
...
def _load_dotenv() -> None:
    """The Databricks SDK does not read `.env`; the other scripts do this too."""
    try:
...
def _store(settings: Settings) -> ConfigStore:
    ...
    if source is None:
        raise SystemExit(
            "No Postgres is configured, so there is no configuration table to publish to.\n"
            "Set LAKEBASE_INSTANCE (deployed shape) or SUPERVISOR_PG_DSN (local docker "
            "Postgres) and try again."
        )
    return ConfigStore(source, settings.config_table)
...
def main() -> int:
    ...
    print(
        ...
        f"on {os.getenv('LAKEBASE_INSTANCE') or 'the configured DSN'}\n"
    )
