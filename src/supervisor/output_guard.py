...
class OutputGuard:
    ...
    def __init__(
        ...
        self._rules = [
            (
                ...
                rule.get("reason", "matched an output policy rule"),
                rule.get("action", BLOCK) if rule.get("action") in (BLOCK, ESCALATE) else BLOCK,
            )
    ...
    def screen(self, text: str) -> OutputScreenResult:
        ...
        for regex, reason, action in self._rules:
            if regex.search(raw):
                return OutputScreenResult(text="", action=action, reason=reason)
    ...
    def stream_should_hold(self, buffer: str) -> bool:
        ...
        if "-----BEGIN" in buffer:
            return True
        for regex, _reason, _action in self._rules:
