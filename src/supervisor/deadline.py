...
────────────────────
One budget for the turn, drawn down by every outbound call:

  * `ensure()` before a call — a call that cannot finish in time never starts,
    so a fan-out or a retry loop stops instead of overrunning;
  * `cap()` on the call's own timeout — the call itself cannot run past the
    deadline either.

With both, the worst case is the budget, not the budget plus one more call.

`BudgetExhausted` is deliberately a distinct type rather than a `TimeoutError`:
`dispatch._is_transient` matches on "timeout", and an exhausted budget must not
...
class Deadline:
    ...
    infinity, `ensure()` never raises and `cap()` returns the caller's own
    limit unchanged. That is the path every offline test and every direct graph
    ...
    def cap(self, seconds: float) -> float:
        """`seconds`, reduced to what the budget can still afford.

        Rounded up to a whole second because the value keys the chat-model cache
        in `model_provider._cached_chat_model`; a float that changes on every
        call would make every call a cache miss and rebuild the client. Never
        returns less than one second — a sub-second timeout is a guaranteed
        failure dressed up as a bound, and `ensure()` is what refuses a call
        with no time left.
        """
        if not self.enabled:
            return seconds
        ...
        if left >= seconds:
            return seconds
        return float(max(1, math.ceil(left)))
