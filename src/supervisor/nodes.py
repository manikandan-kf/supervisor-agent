...
from . import grounding, progress, review_queue, session_notes
from .context import SupervisorContext
from .deadline import BudgetExhausted, deadline_for
from .dispatch import WorkerUnavailable
from .guardrails import followup_answer, small_talk_kind
from .rbac import DENIED_MESSAGE
from .redact import redact_structure, redact_text
from .review_queue import ReviewQueueError
...
def _noted_reply(note: str, held: int) -> str:
...
def _sensitive_prose(labels) -> str:
...
def _context_prose(carried: dict) -> str:
...
_CLARIFY_RETRY_PREFIX = "That doesn't tell me yet — "
_CLARIFY_FINAL_PREFIX = "Last check before I bring in a human reviewer — "
...
def _small_talk_reply(kind: str, reachable: list[str], seen: int = 0) -> str:
...
class SupervisorNodes:
    ...
    def guardrails(
        ...
        if not result.passed:
            ...
            covered = _join([a.name for a in reachable])
            ...
            return Command(
                ...
                update={
                    **spent,
                    ...
                    "routed_agent_name": "",
                    "final_text": _noted_reply(note, len(kept)),
                    "audit_trail": _trail(
                        state,
                        "guardrails",
                        "noted",
    ...
    def _clarify_or_escalate(
        ...
        if question and state.get("pending_clarification"):
            ...
            text = (_CLARIFY_FINAL_PREFIX if final_attempt else _CLARIFY_RETRY_PREFIX) + question
    ...
    def dispatch(
        ...
        if relay_masked:
            text = (
                text.rstrip()
                + "\n\n"
                + SENSITIVE_INPUT_NOTICE.format(what=_sensitive_prose(relay_masked))
            )
        ...
        if carried_over:
            text = text.rstrip() + CARRIED_CONTEXT_NOTICE.format(
                what=_context_prose(carried_over)
            )
