...
Before this module the supervisor did the faithful-log-entry version: a block
offered `appeal`, the appeal was accepted and written to the decision trail with
`outcome: "escalated"`, and nothing ever read it. Three separate controls
depended on that missing half — Stage 03's appeal path, Stage 03's
end-to-end appeal audit (*"who reviewed it, what they decided, and what that
decision then changed"*), and Stage 04's *"make that state genuinely terminal
until the review resolves"*.

The three properties, and where each one lives
──────────────────────────────────────────────
**Enumerable.** `list_open()` — one indexed query. The reviewer surface is the
gateway's `/api/v1/reviews` endpoints, which is where authorization already
lives; this module deliberately holds no opinion about who may call it.
...
**It changes what happens next.** A resolution carries a `decision`:
  ...
  * `allow_retry` — the reviewer judged the request in scope. The next turn on
    that conversation gets **one** pass through the guardrail screen, consumed
...
request path for ordinary traffic: `nodes` consults it only when the
...
must not proceed unreviewed — and has no effect at all on everyone else. The
check costs one query per turn only for conversations under review.
"""
...
# A reviewer's working set is "what is open, oldest first"; the supervisor's
# per-turn question is "is anything open for *this* conversation". One index
# each, and the partial index keeps the reviewer query off the resolved history.
_INDEXES = (
...
def _row_to_review(row) -> Review:
    ...
    `psycopg` is configured with a dict row factory in some paths and not in
    others (see `memory.get_connection`), and the audit sink already carries the
    same defensive read. Handling both here keeps this module usable from a test
    with a plain fake cursor.
    """
...
class ReviewQueue:
    ...
    def __init__(self, connection_source, table: str = "supervisor_review_queue"):
    ...
    def _ensure_table(self, conn) -> None:
        ...
        with conn.cursor() as cur:
            ...
            # search_path — see `config_store.table_exists_here`.
    ...
    def open_for_conversation(self, conversation_id: str) -> Optional[Review]:
        """The oldest still-open review on this conversation, if any.

        This is the per-turn question, and it is only asked for conversations
        whose own checkpointed state already says a review is open — see the
        module docstring on availability.
        """
        if not conversation_id:
            return None
        try:
            ...
            raise ReviewQueueError(
                f"could not check reviews for {conversation_id}: {exc}"
            ) from exc
        return _row_to_review(row) if row is not None else None
...
class NullReviewQueue:
    ...
    Every write **raises**, which is the point: without a durable queue the
    supervisor must not tell a user their appeal reached a human. Reads report
    nothing open, so an offline run behaves exactly like a system with an empty
    queue rather than one that refuses every turn.
    """

    def open_review(self, **_kwargs) -> Review:
        raise ReviewQueueError(
            "no review queue is configured — an appeal cannot be recorded, so it "
            "must not be reported as flagged. Configure SUPERVISOR_PG_DSN or Lakebase."
        )
...
def build_review_queue(settings):
    """The queue backed by the same Postgres the audit sink uses, or a null one."""
    ...
    if source is None:
        logger.warning(
            ...
            "Configure SUPERVISOR_PG_DSN or Lakebase."
        )
