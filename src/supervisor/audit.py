"""Response & audit stage sink — BR-006.
...
Three sinks, chosen by what is configured (see `settings.py` for the order and
the reasoning). `PostgresAuditLogger` is the one to prefer: it writes to the
same Postgres the checkpointer uses (on Lakebase, via a pooled connection with
rotating credentials), needs no additional entitlement, and works from inside
Model Serving — unlike the SQL warehouse path, whose INSERTs are refused
because the endpoint's service principal cannot be granted
...
# migration an owner runs — `scripts/provision_lakebase.py --migrate-audit`.
#
...
# --restrict-governance-tables` revokes UPDATE/DELETE), and the chain head can
...
class PostgresAuditLogger:
    ...
    def __init__(self, connection_source, table: str):
        from .config_store import safe_identifier
        ...
        # interpolation rests on an enforced property rather than on
        # `AUDIT_PG_TABLE` being trustworthy — see `config_store.safe_identifier`.
    ...
    def _ensure_table(self, conn) -> None:
        ...
        with conn.cursor() as cur:
            from .config_store import table_exists_here

            # Scoped to the schema the DDL below would write to, not to the whole
            # search_path — see `config_store.table_exists_here`. Getting this
            # wrong appends this environment's decision trail to another
            # environment's table.
            if not table_exists_here(cur, self._table):
                ...
                if missing:
                    ...
                    logger.error(
                        ...
                        "persisted until scripts/provision_lakebase.py --migrate-audit "
...
class DeltaAuditLogger:
    """Appends one row per request to the UC audit table."""

    def __init__(self, table: str, warehouse_id: str, workspace_client=None):
        self._table = table
    ...
    def log(self, record: dict) -> None:
        ...
        statement = (
            ...
            "(event_time, request_id, conversation_id, user_role, target_agent_id, outcome, decision_trail) "
            "VALUES (:event_time, :request_id, :conversation_id, :user_role, :target_agent_id, "
            ":outcome, :decision_trail)"
        )
        parameters = [
            ...
            StatementParameterListItem(name="request_id", value=str(record.get("request_id", ""))),
            StatementParameterListItem(name="conversation_id", value=str(record.get("conversation_id", ""))),
            StatementParameterListItem(name="user_role", value=str(record.get("user_role", ""))),
            StatementParameterListItem(name="target_agent_id", value=str(record.get("target_agent_id", ""))),
            StatementParameterListItem(name="outcome", value=str(record.get("outcome", ""))),
            StatementParameterListItem(
                name="decision_trail",
                value=json.dumps(record.get("decision_trail", []), default=str),
            ),
        ]
        response = self._client().statement_execution.execute_statement(
            warehouse_id=self._warehouse_id, statement=statement, parameters=parameters
        )
        # execute_statement submits asynchronously and reports SQL failures
        # (e.g. missing UC grants for the endpoint's service principal) in the
        # response status rather than raising — surface those.
        status = getattr(response, "status", None)
        state = getattr(getattr(status, "state", None), "value", None)
        if state in ("FAILED", "CANCELED", "CLOSED"):
            message = getattr(getattr(status, "error", None), "message", "unknown error")
            raise RuntimeError(f"audit insert {state}: {message}")
...
def build_audit_logger(settings):
    """Pick the best sink available. See `settings.py` for the ordering rationale."""
    from .memory import audit_connection_source

    source = audit_connection_source()
    if source is not None:
        logger.info("audit sink: Postgres table %s", settings.audit_pg_table)
        return TracingAuditLogger(PostgresAuditLogger(source, settings.audit_pg_table))

    if settings.audit_warehouse_id:
        # This path cannot work from inside Model Serving — the endpoint's
        # system service principal can never hold `databricks-sql-access`, so
        # every INSERT is refused, governance turns are held and answers go
        # unaudited. Selecting it used to take nothing more than setting
        # AUDIT_WAREHOUSE_ID; now it also takes an explicit acknowledgement,
        # so a plausible-looking operator action cannot silently break every
        # audit write.
        if getattr(settings, "audit_allow_warehouse", False):
            logger.info("audit sink: SQL warehouse -> %s", settings.audit_table)
            return TracingAuditLogger(
                DeltaAuditLogger(settings.audit_table, settings.audit_warehouse_id)
            )
        logger.error(
            "AUDIT_WAREHOUSE_ID is set but the SQL-warehouse audit path is "
            "unsupported from Model Serving (the endpoint's service principal "
            "cannot hold databricks-sql-access) — ignoring it. Set "
            "AUDIT_ALLOW_WAREHOUSE=true only if you are certain this identity "
            "can execute warehouse statements."
        )

    logger.warning(
        "audit sink: process log only — the decision trail will not be queryable, "
        "so BR-006 is not met. Configure SUPERVISOR_PG_DSN or Lakebase."
    )
